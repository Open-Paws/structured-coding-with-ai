...
Run the relevant test subset before each commit. Every commit must leave the codebase passing. If tests fail, fix before committing — do not push broken commits to shared branches. For advocacy code handling investigation or evidence data, verify that no sensitive data has leaked into test output, logs, or error messages.

### Step 4: Write the Commit Message
Write messages that explain WHY, not WHAT — the code shows what changed. First line: 50 characters, imperative mood. Reference the issue or ticket. Add AI attribution trailers so the team knows which code was agent-generated.
...
- **Suggestion acceptance rate** — healthy range is 25-35%. Higher may indicate over-reliance on AI without critical review. In advocacy projects, over-reliance means unreviewed security assumptions.
