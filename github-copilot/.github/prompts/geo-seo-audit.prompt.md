...
**Wikipedia COI requirements (mandatory):** Affiliated team members must not directly edit Wikipedia articles about their own organization. All edits must use the Talk page to request changes, disclosing affiliation explicitly. Content must cite only independent reliable sources — organizational websites and press releases do not establish notability. All COI disclosures, Talk-page requests, and independent editor engagements must be documented in the audit report. These steps are required before flagging Wikipedia creation as an action item.
