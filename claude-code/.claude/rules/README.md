...
Each rule file can include optional `paths:` YAML frontmatter. When the file you are editing matches a rule's glob pattern, Claude Code includes that rule in context. Files without `paths:` frontmatter (like `cost-optimization.md`, `advocacy-domain.md`, and `desloppify.md`) are loaded based on task context rather than file path.
...
| `desloppify.md` | Code quality workflow, scan loop, backlog management, and execution discipline | (no path filter) |
